...
for software_name in ${PROGRAMS_TO_INSTALL[@]}; do
  if ! dpkg -l | grep -q $software_name; then # Só instala se já não estiver instalado
    apt install "$software_name" -y
  else
    echo "[Programa instalado com sucesso] - $software_name"
  fi
done
